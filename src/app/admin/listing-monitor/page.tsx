...
import { Eye, Play, Loader2, AlertCircle, Clock } from "lucide-react";
...
export default function ListingMonitorPage() {
  ...
  const runScan = async () => {
    ...
    setError(null);
    try {
      const data = await sweetleaseApi.post("/api/admin/intelligence/listing-monitor", {
  ...
  const resultArray = results
    ...
    : results?.listings || results?.results || results?.data || []
    : [];
    ...
    <div className="max-w-6xl">
      ...
      {loading && (
        <div className="bg-white border border-slate-200 rounded-xl p-12 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-slate-500">Scanning listings across 7 cities...</p>
        </div>
      )}
      ...
      {!loading && !results && !error && (
        <div className="bg-white border border-slate-200 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-slate-500">Click &ldquo;Run Scan&rdquo; to check listings across target cities.</p>
        </div>
      )}
    </div>
  );
