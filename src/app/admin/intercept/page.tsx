...
import { MessageCircle, Play, Loader2, AlertCircle, Clock } from "lucide-react";
...
export default function InterceptPage() {
  ...
  const runScan = async () => {
    ...
    setError(null);
    try {
      const data = await sweetleaseApi.post("/api/admin/intelligence/intercept", { limit: 50 });
      setResults(data);
  ...
  const resultArray = results
    ...
    : results?.targets || results?.messages || results?.results || results?.data || []
    : [];
    ...
    <div className="max-w-6xl">
      ...
      {loading && (
        <div className="bg-white border border-slate-200 rounded-xl p-12 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-slate-500">Scanning intercept targets...</p>
        </div>
      )}
      ...
      {!loading && !results && !error && (
        <div className="bg-white border border-slate-200 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-slate-500">Click &ldquo;Run Scan&rdquo; to find intercept targets with generated messages.</p>
        </div>
      )}
    </div>
  );
